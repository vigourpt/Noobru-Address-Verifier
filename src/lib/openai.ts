...
RESPOND ONLY WITH THE JSON OBJECT, NO OTHER TEXT.`
    ...
    let verifiedAddress = JSON.parse(response.choices[0].message.content || '{}');

    // Post-process to verify ranges if necessary
    if (verifiedAddress.country === "Australia" && !verifiedAddress.address1.includes('-')) {
      const inferredRangeResponse = await openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
          {
            role: "system",
            content: `You are a knowledgeable assistant for Australian addresses. Determine if the address "${verifiedAddress.address1}" should contain a street number range based on your internal knowledge. Respond only with the corrected address.`
          }
        ],
        temperature: 0.3,
        max_tokens: 100
      ...
      const inferredRange = inferredRangeResponse.choices[0]?.message?.content?.trim();
      if (inferredRange) {
